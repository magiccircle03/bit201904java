...
<div id="contents">
	...
	<table>
		<tr>
		<!-- 인덱스는 삭제되는 경우 번호 건너띄고 그러는데 그걸 처리하는 거 이따 할거임 -->
			<td>no</td>
			<td>아이디</td>
			<td>비밀번호</td>
			<td>이름</td>
		...
		<!-- 실제론 getMemberList() 부르는 것. el이었죠 -->
		<c:forEach items="${viewData.memberList}" var="memberInfo" varStatus="stat">
		<tr>
		<!-- 사실 연번형태로 이런건 의미없기때문에 요즘 사용하진 않지만 필요하다면 이렇게 하면 됨 -->
			<td>${viewData.no-stat.index} </td> 
			<td>${memberInfo.uId}</td><!-- getuId() 부르는 것  -->
			<td>${memberInfo.uPW}</td>
			<td>${memberInfo.uName}</td>
			<td><img src="<c:url value='/uploadfile/userphoto/${memberInfo.uPhoto}'/>" height="30"></td><!-- 파일 존재안하면 디폴트로 변경해주는 처리도 할 것, 30이라고 주면 30px -->
			<td>
				${memberInfo.regDate}
